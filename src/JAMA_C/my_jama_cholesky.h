...
/** 
   ...
   05_July_04, I download this folder from TNT website http://math.nist.gov/tnt/,
        then I change 'Array1D, Arra2D and Real' to 'Vector, Matrix and double'.
   */
...
Cholesky::Cholesky(const Matrix<double> &A)
{
	...
	for (int j = 0; j < n; j++) {
	    double d = 0.0;
            for (int k = 0; k < j; k++) {
		double s = 0.0;
		for (int i = 0; i < k; i++) { s += L_[k][i]*L_[j][i]; }
	    ...
	    for (int k = j+1; k < n; k++)  { L_[j][k] = 0.0; }
	}
}
...
// Cleve's original code
      ...
      for (int k = 0; k < n; k++) {
         for (int i = k+1; i < n; i++) {
            for (int j = 0; j < nx; j++) { X[i][j] -= X[k][j]*L_[k][i]; }
         }
         for (int j = 0; j < nx; j++)  { X[k][j] /= L_[k][k]; }
      }
...
/** Return the inverse of the matrix. If it is not square matrix, abort.
 ** (Write on 05_Jul_04)
 **	this function does not work. --- 06-May-27
 **
Matrix<double> Cholesky::inv()
{
	assert( L_.num_rows() == L_.num_cols() );
	int n = L_.num_rows();
